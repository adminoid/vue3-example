...
.left-sidebar(:class="{'left-sidebar_open': sidebarOpen}")
  ...
  a.left-sidebar__sticker(v-if="!isMobileViewport && sidebarOpen" @click.prevent="togglePin")
    svg(v-if="sidebarPinned" width='17px' height='17px' xmlns='http://www.w3.org/2000/svg')
      g#pin(transform='translate(-302.000000, -174.000000)' fill='#A9A9A9')
        path(:d="pinnedIconD"
          transform='translate(309.000000, 184.100000)'
          +' rotate(-315.000000) translate(-309.000000, -184.100000)'
        )
    svg(v-else width='14px' height='14px' xmlns='http://www.w3.org/2000/svg')
      g#pin(transform='translate(-303.000000, -176.000000)' fill='#A9A9A9')
        path(:d="unpinnedIconD"
          transform='translate(309.000000, 184.000000)'
          +' rotate(-315.000000) translate(-309.000000, -184.000000)'
        )
...
const LeftSidebar = defineComponent({
  ...
  setup () {
    ...
    watch(
      () => store.getters.isMobileViewport,
      (value) => {
        if (value) {
          if (store.state.layout.chat.open) sidebarClose()
          unpinSidebar()
